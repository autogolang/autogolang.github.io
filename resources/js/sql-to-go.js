var sql2go = {
  ...
  to_gostruct: function () {
    ...
    let res = val.match(/\`[\w_]+\`\s*[\w_\(\)]+.*(\s+|\,)/gi)
    if (!res) {
    ...
    for (var i = 0, len = res.length; i < len; i++) {
      var fieldReg =
        /\`(.+)\`\s*(tinyint|smallint|int|mediumint|bigint|float|double|decimal|varchar|char|text|mediumtext|longtext|datetime|time|date|enum|set|blob|json)?/
