...
$(function () {
  ...
  function jsonConversion() {
    ...
    window.history.replaceState(
      {},
      '',
      changeURLArg('schema', encodeURIComponent($schema.text().replace(/\n/g, '%0A')))
    )
    window.history.replaceState({}, '', changeURLArg('url', encodeURIComponent($url.text())))
  }
  ...
  function forward(opt) {
    ...
    fetch(forwardUrl, {
      method: 'POST',
      headers: opt.headers,
      body: 'method=' + opt.method + '&url=' + encodeURIComponent($url.text()) + '&body=' + opt.body,
    })
      .then((response) => response.json())
      .then((data) => {
        $json.text(stringify(data))
